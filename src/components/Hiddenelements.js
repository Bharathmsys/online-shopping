...
//   const d = JSON.parse(localStorage.getItem("dd")) || JSON.parse(localStorage.getItem("abcd"))
  ...
  return (
    <div>
     ...
     {JSON.parse(localStorage.getItem("dd")).length<1?<a href="/"><button className="btn btn-success">Back to Home </button></a>:<Link to="/Checkout-Products"><button className="btn btn-success">Place Order</button></Link>}
      </div>
    </div>
  );
};
